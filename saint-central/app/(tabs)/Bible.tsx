...
const getBookCategory = (book: string): string => {
  ...
  const historyBooks = [
    ...
    "Tobit", "Judith", "1 Maccabees", "2 Maccabees", "Additions to Esther"
  ];
  const wisdomBooks = [
...
// Removed duplicate handleVersionSelect function
  ...
  // ---------------------
  const addFavorite = async (book: string, chapter: string, verse: string, note: string = ""): Promise<void> => {
    try {
  ...
  const removeFavorite = async (book: string, chapter: string, verse: string): Promise<void> => {
    try {
      ...
      setFavorites(prev =>
        prev.filter(fav => !(fav.book === book && fav.chapter === chapter && fav.verse === verse))
      );
  ...
  const updateFavoriteNote = async (book: string, chapter: string, verse: string, note: string): Promise<void> => {
    try {
      // Update state first
      setFavorites(prev => {
        return prev.map(fav => {
  ...
  const getFilteredBooks = useCallback(() => {
    ...
    if (includesDeuterocanonical(selectedVersion)) {
      if (testament === "old") {
        ...
        filteredBooks = [...bibleBooks.oldTestament, ...deuterocanonicalBooks, ...bibleBooks.newTestament];
      }
    } else {
  ...
  const isFavorite = useCallback((book: string, chapter: string, verse: string): boolean => {
  ...
  const isChapterBookmarked = useCallback((book: string, chapter: string): boolean => {
    return favorites.some(
      fav => fav.book === book && fav.chapter === chapter && fav.verse === "0"
    );
  }, [favorites]);
                ...
                {bibleVersions.map((version) => (
                  ...
                  >
                    <View style={[styles.themeIconContainer, { 
                      ...
                      borderColor: selectedVersion === version.table 
                        ? themeStyles.accentColor 
                        : 'transparent'
                    }]}>
                      <Feather 
                    ...
                    <View style={styles.versionTextContainer}>
                      <Text style={[
                        styles.themeOptionText, 
                        { 
                          color: selectedVersion === version.table 
                            ? themeStyles.accentColor 
                            : themeStyles.textColor
                        }
                      ]}>
          ...
          >
            ...
            >
              ...
              <View style={[styles.noteContainer, {
                ...
                <Text style={[styles.noteLabel, { color: themeStyles.textColor }]}>
                  Note:
                </Text>
                <TextInput
    ...
    <View style={styles.favoritesContainer}>
      {favoriteLoading ? (
        ...
        <View style={styles.emptyFavoritesContainer}>
          ...
          >
            <Text style={styles.emptyFavoritesButtonText}>
              Browse Bible
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View>
          ...
          {Object.entries(getGroupedFavorites()).map(([group, items]) => (
            <View key={group} style={styles.favoriteGroup}>
              <Text style={[styles.favoriteGroupTitle, { 
                color: themeStyles.textColor,
              ...
              {items.map((favorite, index) => (
                ...
                >
                  ...
                  <View style={styles.favoriteItemContent}>
                    ...
                    {favorite.note ? (
                      <Text style={[styles.favoriteItemNote, { 
      ...
      {favorites.length > 0 && (
        <View style={styles.highlightedBookmarksSection}>
          ...
          <FlatList<FavoriteItem>
              ...
              >
                ...
                <View style={styles.bookmarkTextContainer}>
                  <Text style={[
                    ...
                    {item.book} {item.chapter}{item.verse !== "0" && `:${item.verse}`}
                  </Text>
            ...
            keyExtractor={(item, index) => `bookmark-${item.book}-${item.chapter}-${item.verse}-${index}`}
            horizontal
              ...
              >
                ...
                <View style={styles.recentItemActions}>
                  {isChapterBookmarked(item.book, item.chapter) ? (
                    <TouchableOpacity
                      style={[styles.bookmarkedRecentButton, {
                        backgroundColor: `${themeStyles.favoriteColor}15`
                      }]}
                    ...
                    <TouchableOpacity
                      style={[styles.bookmarkRecentButton, {
                        borderColor: themeStyles.borderColor
                      }]}
        ...
        <View style={styles.chaptersGrid}>
          {chapters.map((chapter) => (
            ...
            >
              ...
              {isChapterBookmarked(selectedBook!, chapter) && (
                <View style={[styles.chapterBookmarkIndicator, {
                  backgroundColor: themeStyles.favoriteColor
                }]}>
            ...
            <TouchableOpacity 
              style={[
                ...
                { 
                  ...
                  backgroundColor: fontSize !== 'small' 
                    ? `${themeStyles.accentColor}15` 
                    : 'transparent',
                  borderColor: themeStyles.borderColor
                }
            ...
            <TouchableOpacity 
              style={[
                ...
                { 
                  ...
                  backgroundColor: fontSize !== 'xlarge' 
                    ? `${themeStyles.accentColor}15` 
                    : 'transparent',
                  borderColor: themeStyles.borderColor
                }
        ...
        <View style={styles.versesContent}>
          ...
          {verses.length > 0 && (
            <View style={styles.chapterEndNavigationContainer}>
              <TouchableOpacity
                style={[styles.chapterEndNavButton, {
                  backgroundColor: themeStyles.cardColor,
                  borderColor: themeStyles.borderColor
                }]}
                onPress={goToPrevChapter}
              ...
              >
                ...
                />
                <Text style={[styles.chapterEndNavText, {
                  color: chapters.indexOf(selectedChapter!) > 0 
                    ? themeStyles.accentColor
                    : `${themeStyles.textColor}40`
                }]}>
                  Previous Chapter
              ...
              >
                <Text style={[styles.chapterEndNavText, {
                  color: chapters.indexOf(selectedChapter!) < chapters.length - 1
                    ? themeStyles.accentColor
                    : `${themeStyles.textColor}40`
                }]}>
                  Next Chapter
  ...
  const searchBible = async (e: NativeSyntheticEvent<TextInputSubmitEditingEventData>): Promise<void> => {
    setSearching(true);
    try {
      const { data, error } = await supabase
        .from(selectedVersion)
                  ...
                  <TouchableOpacity 
                    style={[
                      ...
                      isChapterBookmarked(selectedBook!, selectedChapter!) && 
                        [styles.activeHeaderButton, { backgroundColor: `${themeStyles.favoriteColor}20` }]
                    ]}
            ...
            <View style={styles.favoritesFilterButtons}>
              ...
              >
                <Text style={[
                  ...
                  { color: favoriteGrouping === "book" ? themeStyles.favoriteColor : themeStyles.textColor }
                ]}>
                  Book
                </Text>
              </TouchableOpacity>
              ...
              >
                <Text style={[
                  styles.favoritesFilterButtonText,
                  { color: favoriteGrouping === "date" ? themeStyles.favoriteColor : themeStyles.textColor }
                ]}>
                  Date
                </Text>
              </TouchableOpacity>
              ...
              >
                <Text style={[
                  styles.favoritesFilterButtonText,
                  { color: favoriteGrouping === "none" ? themeStyles.favoriteColor : themeStyles.textColor }
                ]}>
                  None
                </Text>
              </TouchableOpacity>
