...
const GUIDE_AUDIO_FILES = {
  "Francis": {
    ...
    CONCLUSION: require('../../assets/audio/rosary1.mp3'),
  },
  "Claire": {
    ...
    CONCLUSION: require('../../assets/audio/rosary2.mp3'),
  },
  "Thomas": {
    INTRODUCTION: require('../../assets/audio/rosary1.mp3'),
    JOYFUL: require('../../assets/audio/rosary1.mp3'),
...
const getMysteryTheme = (mysteryKey: string) => {
  switch (mysteryKey) {
    ...
    case "CONCLUSION":
      return {
        primary: "#FF4757", // Using Sorrowful theme for conclusion
        secondary: "#D63031",
        accent: "#FFE9EB",
        gradientStart: "#FF4757",
        gradientEnd: "#D63031",
...
// Sorrowful Mysteries content for summary
const SORROWFUL_MYSTERIES = [
  { 
    title: "The Agony in the Garden", 
    description: "Then Jesus came with them to a place called Gethsemane, and he said to his disciples, 'Sit here while I go over there and pray.' He took along Peter and the two sons of Zebedee, and began to feel sorrow and distress. (Matthew 26:36-37)"
  },
  { 
  ...
  { 
    ...
    description: "And the soldiers wove a crown out of thorns and placed it on his head, and clothed him in a purple cloak, and they came to him and said, 'Hail, King of the Jews!' And they struck him repeatedly. (John 19:2-3)"
  },
  { 
    title: "The Carrying of the Cross", 
    description: "And carrying the cross himself, he went out to what is called the Place of the Skull, in Hebrew, Golgotha. (John 19:17)"
  },
  { 
    title: "The Crucifixion", 
    description: "There they crucified him, and with him two others, one on either side, with Jesus in the middle. (John 19:18)"
  }
];

// Conclusion content
const CONCLUSION_CONTENT = {
  title: "Conclusion of the Sorrowful Mysteries",
  description: "We have meditated on Christ's passion and suffering. Through the Sorrowful Mysteries, we unite our own sufferings with Christ's and find meaning in His redemptive love."
};
...
export default function SorrowfulPrayer7Screen() {
  ...
  // State for prayer interface - starting with the first concluding prayer
  ...
  // State for showing summary of mysteries
  const [showMysteriesSummary, setShowMysteriesSummary] = useState(false);
  ...
  const toggleAudio = async () => {
    try {
      if (isPlaying) {
        ...
        if (!result) {
          // If no audio is loaded, play the conclusion
          await audioManager.playMystery("CONCLUSION", parseInt(mysteryIndex as string) || 5);
        }
        setIsPlaying(true);
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          >
            ...
            <Text style={styles.mysteryTitle}>{CONCLUSION_CONTENT.title}</Text>
            <View style={styles.mysteryReflectionContainer}>
              <Text style={styles.mysteryReflection}>{CONCLUSION_CONTENT.description}</Text>
            </View>
            
            <TouchableOpacity
              style={styles.mysterySummaryButton}
              onPress={() => setShowMysteriesSummary(true)}
            >
              <Text style={styles.mysterySummaryButtonText}>View All Sorrowful Mysteries</Text>
            </TouchableOpacity>
          </Animated.View>
          ...
          <View style={styles.navigationContainer}>
            ...
            >
              <Text style={[styles.navButtonText, { color: theme.primary }]}>
                {currentPrayerStep === prayers.length - 1 ? "FINISH" : "NEXT"}
              </Text>
            ...
            <ScrollView style={styles.fullPrayerScrollView}>
              <View style={styles.fullPrayerContent}>
                <Text style={styles.fullPrayerText}>
                  ...
                  {currentPrayerStep === 3 && "In the name of the Father, and of the Son, and of the Holy Spirit. Amen."}
                </Text>
              </View>
            </ScrollView>
      ...
      {/* Mysteries Summary Modal */}
      <Modal
        visible={showMysteriesSummary}
        transparent={true}
        animationType="fade"
        onRequestClose={() => setShowMysteriesSummary(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.fullPrayerModalCard, { maxHeight: height * 0.8 }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>The Sorrowful Mysteries</Text>
              <TouchableOpacity 
                style={styles.modalCloseButton}
                onPress={() => setShowMysteriesSummary(false)}
              >
                <AntDesign name="close" size={20} color="#333" />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={styles.fullPrayerScrollView}>
              <View style={styles.fullPrayerContent}>
                {SORROWFUL_MYSTERIES.map((mystery, index) => (
                  <View key={index} style={styles.mysterySummaryItem}>
                    <Text style={styles.mysterySummaryNumber}>{index + 1}</Text>
                    <View style={styles.mysterySummaryContent}>
                      <Text style={styles.mysterySummaryTitle}>{mystery.title}</Text>
                      <Text style={styles.mysterySummaryDescription}>{mystery.description}</Text>
                    </View>
                  </View>
                ))}
              </View>
