...
var tokenBlacklist = /* @__PURE__ */ new Map();
...
async function generateSecureToken(lengthBytes = 32) {
  return generateRandomBytes(lengthBytes);
}
__name(generateSecureToken, "generateSecureToken");
async function createCsrfToken(userId) {
...
async function securityMiddleware(request, env, options = {}) {
  const {
    requireAuth = false,
    rateLimitByIp = true,
    ...
    if (!ipLimitResult.allowed) {
      return {
        ...
        error: createResponse(
          ...
          429,
          { "Retry-After": Math.ceil(ipLimitResult.retryAfter / 1e3).toString() }
        )
      };
    }
  }
  if (requireAuth || rateLimitByToken || validateCsrf) {
      ...
      if (!tokenLimitResult.allowed) {
        return {
          ...
          error: createResponse(
            ...
            429,
            { "Retry-After": Math.ceil(tokenLimitResult.retryAfter / 1e3).toString() }
          )
        };
      }
    }
    try {
      ...
      const newCsrfToken = validateCsrf ? await createCsrfToken(user.user.id) : void 0;
      return {
        isAuthorized: true,
        userId: user.user.id,
        clientIp,
...
function checkRateLimit(key, customConfig) {
  const now = Date.now();
...
function createResponse(body, status, additionalHeaders = {}) {
...
var TABLE_PERMISSIONS = {
  users: {
    ownerOnly: true,
    selfTable: true,
    // Special case: user's own ID is in the "id" field
    allowedColumns: [
      "id",
      "email",
      "first_name",
...
// src/server/dataHandler.ts
var activeTransactions = /* @__PURE__ */ new Map();
var TRANSACTION_TIMEOUT = 3e5;
...
__name(cleanupExpiredTransactions, "cleanupExpiredTransactions");
...
async function handleSelect(request, env) {
  try {
    ...
    if (request.method === "GET") {
      ...
      const offset = url.searchParams.get("offset") ? parseInt(url.searchParams.get("offset")) : void 0;
      let range;
    ...
    if (transactionId && activeTransactions.has(transactionId)) {
      ...
      const result2 = await executeSupabaseQueryWithTransaction(
        tx.supabase,
        selectParams,
        tx.userId
      );
    ...
    return createResponse(
      {
        ...
        count: result.count,
        params: selectParams
        // Include for debugging/transparency
      },
      200
    );
  } catch (error) {
...
async function handleInsert(request, env) {
  ...
  try {
    ...
    if (transactionId && activeTransactions.has(transactionId)) {
      ...
      const tablePermissions2 = TABLE_PERMISSIONS[insertParams.table] || {};
      if (tx.userId && tablePermissions2.ownerOnly) {
        const ownerIdColumn = tablePermissions2.ownerIdColumn || "user_id";
      ...
      const { data: data2, error: dbError2 } = await tx.supabase.from(insertParams.table).insert(insertParams.data).select();
      if (dbError2) {
        return createResponse(
          {
            error: "Database error",
            message: dbError2.message,
            details: dbError2
          },
          500
        );
    ...
    if (tablePermissions.allowedColumns) {
      const checkColumnPermissions = /* @__PURE__ */ __name((data2) => {
        const columns = Object.keys(data2);
    ...
    const { data, error: dbError } = await supabase.from(insertParams.table).insert(insertParams.data).select();
    if (dbError) {
...
async function handleUpsert(request, env) {
  ...
  try {
    ...
    if (transactionId && activeTransactions.has(transactionId)) {
      ...
      const tablePermissions2 = TABLE_PERMISSIONS[upsertParams.table] || {};
      if (tx.userId && tablePermissions2.ownerOnly) {
        const ownerIdColumn = tablePermissions2.ownerIdColumn || "user_id";
      ...
      const upsertOptions2 = upsertParams.onConflict && upsertParams.onConflict.length > 0 ? { onConflict: upsertParams.onConflict.join(",") } : {};
      const { data: data2, error: dbError2 } = await tx.supabase.from(upsertParams.table).upsert(upsertParams.data, upsertOptions2).select();
      if (dbError2) {
        return createResponse(
          {
            error: "Database error",
    ...
    const upsertOptions = upsertParams.onConflict && upsertParams.onConflict.length > 0 ? { onConflict: upsertParams.onConflict.join(",") } : {};
    const { data, error: dbError } = await supabase.from(upsertParams.table).upsert(upsertParams.data, upsertOptions).select();
    if (dbError) {
      console.error("Database error:", dbError);
      return createResponse(
        {
...
async function handleUpdate(request, env) {
  ...
  try {
    ...
    if (transactionId && activeTransactions.has(transactionId)) {
      ...
      const tablePermissions2 = TABLE_PERMISSIONS[updateParams.table] || {};
      let builder2 = tx.supabase.from(updateParams.table).update(updateParams.data);
      if (tablePermissions2.ownerOnly) {
        const ownerIdColumn = tablePermissions2.ownerIdColumn || "user_id";
        if (tablePermissions2.selfTable) {
          builder2 = builder2.eq("id", tx.userId);
        } else if (ownerIdColumn === "special_friendship") {
          builder2 = builder2.or(`user_id_1.eq.${tx.userId},user_id_2.eq.${tx.userId}`);
        } else {
          builder2 = builder2.eq(ownerIdColumn, tx.userId);
        }
      }
      Object.entries(updateParams.where).forEach(([key, value]) => {
        if (value === null) {
          builder2 = builder2.is(key, null);
        } else {
          builder2 = builder2.eq(key, value);
        }
      });
      if (updateParams.filters && updateParams.filters.length > 0) {
        for (const filter of updateParams.filters) {
          const { column, operator, value } = filter;
          applyFilterToBuilder(builder2, column, operator, value);
        }
      }
      const { data: data2, error: dbError2 } = await builder2.select();
      if (dbError2) {
        return createResponse(
          {
            error: "Database error",
...
async function handleDelete(request, env) {
  ...
  try {
    ...
    if (transactionId && activeTransactions.has(transactionId)) {
      ...
      let builder2 = tx.supabase.from(deleteParams.table).delete();
      const tablePermissions2 = TABLE_PERMISSIONS[deleteParams.table] || {};
      if (tablePermissions2.ownerOnly) {
        const ownerIdColumn = tablePermissions2.ownerIdColumn || "user_id";
        if (tablePermissions2.ownerIdColumn === "special_friendship") {
          builder2 = builder2.or(`user_id_1.eq.${tx.userId},user_id_2.eq.${tx.userId}`);
        } else {
          builder2 = builder2.eq(ownerIdColumn, tx.userId);
        }
...
async function handleTransactionStart(request, env) {
  if (request.method !== "POST") {
    return createResponse({ error: "Method not allowed" }, 405);
  }
  try {
...
__name(handleTransactionStart, "handleTransactionStart");
async function handleTransactionCommit(request, env) {
  if (request.method !== "POST") {
    return createResponse({ error: "Method not allowed" }, 405);
  }
  try {
...
__name(handleTransactionCommit, "handleTransactionCommit");
async function handleTransactionRollback(request, env) {
  if (request.method !== "POST") {
    return createResponse({ error: "Method not allowed" }, 405);
  }
  try {
...
__name(handleTransactionRollback, "handleTransactionRollback");
async function applySecurityAndPermissions(request, env, table, operation) {
  ...
  const requireAuth = tablePermissions.ownerOnly || !!tablePermissions.requiredRole || tablePermissions.selfTable || effectiveOperation !== "select";
  const securityResult = await securityMiddleware(request, env, {
    requireAuth,
    rateLimitByIp: true,
    customRateLimit: {
      maxRequests: table === "products" ? 100 : 30,
      // Higher limit for public tables
      windowMs: 6e4
      // 1 minute
    }
  });
  if (tablePermissions.requiredRole && securityResult.isAuthorized && securityResult.userId) {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
    const { data: roleData, error: roleError } = await supabase.from("user_roles").select("role").eq("user_id", securityResult.userId).eq("role", tablePermissions.requiredRole).single();
...
__name(applySecurityAndPermissions, "applySecurityAndPermissions");
function applyFilterToBuilder(builder, column, operator, value) {
  switch (operator) {
    case "eq":
    ...
    case "or":
      if (column === "or" && typeof value === "object") {
        ...
        const options = foreignTable ? { foreignTable } : void 0;
...
__name(applyFilterToBuilder, "applyFilterToBuilder");
async function executeSupabaseQuery(env, params, userId) {
  try {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
...
__name(executeSupabaseQuery, "executeSupabaseQuery");
async function executeSupabaseQueryWithTransaction(supabase, params, userId) {
  try {
    return executeSupabaseQueryWithClient(supabase, params, userId);
  } catch (error) {
...
__name(executeSupabaseQueryWithTransaction, "executeSupabaseQueryWithTransaction");
async function executeSupabaseQueryWithClient(supabase, params, userId) {
  try {
    const tablePermissions = TABLE_PERMISSIONS[params.table] || {};
    const selectQuery = Array.isArray(params.columns) ? params.columns.join(", ") : params.columns || "*";
    let builder = supabase.from(params.table).select(selectQuery, {
      count: params.count === true ? "exact" : void 0
    });
    ...
    if (params.where) {
      const skipKeys = tablePermissions.ownerIdColumn === "special_friendship" ? ["user_id_1", "user_id_2"] : [];
    ...
    if (params.single) {
      ...
      return { data: result.data, error: result.error, count: result.count ?? void 0 };
    }
  } catch (error) {
    console.error("Error executing query:", error);
    return {
...
__name(executeSupabaseQueryWithClient, "executeSupabaseQueryWithClient");
